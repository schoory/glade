...
export const ChannelSettingsPage = () => {
  ...
  useEffect(() => {
    ...
    if (!channelId) { navigate('/channels') } // если в параметре не передается id канала редирект на страницу списка каналов
    request(
      '/api/channels/getcurrentchannel',
      'POST', 
      { userId: user.id, channelId: channelId },
      { Authorization: `Bearer ${user.token}`, Refresh: `Bearer ${user.refreshToken}` }
    ).then(data => {
      if (data.error) {
      ...
      setTabVariables({ name: data.channel.name, type: data.channel.type, permissions: data.channel.visibility, groups: data.channel.groups ? [ ...data.channel.groups ] : [] })
    })
  }, [])
  ...
  const handleSaveSettings = () => {
    ...
    request(
      '/api/channels/savechannelsettings',
      'POST',
      {
        userId: user.id,
        channelId: channelId,
